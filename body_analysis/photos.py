...
def list_monthly_photo_sets(photos_dir: str | None = None) -> Dict[str, List[str]]:
    ...
    if not os.path.isdir(photos_dir):
        return {}
    month_dirs = sorted(
        [d for d in glob.glob(os.path.join(photos_dir, "*")) if os.path.isdir(d)]
    )
    out: Dict[str, List[str]] = {}
    for md in month_dirs:
        month_key = os.path.basename(md)
        images = sorted(
            glob.glob(os.path.join(md, "*.jpg")) + glob.glob(os.path.join(md, "*.png"))
        )
        if images:
...
def list_photos_by_tag(photos_dir: str | None = None) -> Dict[str, Dict[str, str]]:
    """Return mapping tag -> {YYYY-MM-DD: filepath}.

    Expects filenames like: YYYY-MM-DD/face.png, YYYY-MM-DD/profil.png, etc.
    ...
    for dd in date_dirs:
        date_key = os.path.basename(dd)
        # Chercher tous les fichiers images
        images = glob.glob(os.path.join(dd, "*.jpg")) + glob.glob(
            os.path.join(dd, "*.png")
        )
        ...
        for img_path in images:
            ...
            if tag not in tags_data:
                tags_data[tag] = {}
            tags_data[tag][date_key] = img_path

    return tags_data


def load_image(
    image_path: str, blur: bool = False, target_height: int | None = None
) -> Image.Image:
